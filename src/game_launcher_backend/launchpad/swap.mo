...
actor SwapCanister {
    ...
    private stable var _swap_configs : Trie.Trie<Text, Swap.TokenSwapConfigs> = Trie.empty(); // token_canister_id <-> 
    ...
    let icp_ledger : ICP.Self = actor (ENV.Ledger);
    ...
    private func queryIcpTx_(blockIndex : Nat64, toPrincipal : Text, fromPrincipal : Text, amt : ICP.Tokens) : async (Result.Result<Text, Text>) {
        ...
        var blocks : [ICP.CandidBlock] = res.blocks;
        var base_block : ICP.CandidBlock = blocks[0];
        ...
        switch (Trie.find(_swap_participants, Helper.keyT(arg.canister_id), Text.equal)) {
            case (?participants) {
                for((principal, info) in Trie.iter(participants)) {
                    ...
                    let res : ICP.Result = await icp_ledger.icrc1_transfer(req);
                    switch(res) {
                        case (#Ok index) {
    ...
    private func success_mint_tokens(arg : { canister_id : Text; }) : async () {
        ...
        switch(Trie.find(_swap_configs, Helper.keyT(arg.canister_id), Text.equal)) {
            case(?configs) {
                ...
                let icp_res_guilds : ICP.Result = await icp_ledger.icrc1_transfer(icp_req);
                ...
                let icp_res_team : ICP.Result = await icp_ledger.icrc1_transfer(icp_req);
                ...
                let icp_res_liquidity_pool : ICP.Result = await icp_ledger.icrc1_transfer(icp_req);
                ...
                let icp_res_boom_dao : ICP.Result_5 = await icp_ledger.transfer(icp_req_boom_dao);
    ...
    public shared({caller}) func create_icrc_token(init_arg : ICRC.InitArgs) : async ({canister_id : Text;}) {
        assert(caller == dev_principal);
        ...
        switch(Trie.find(_swap_configs, Helper.keyT(arg.canister_id), Text.equal)) {
            case (?configs) {
                ...
                if(configs.swap_start_timestamp_seconds <= current_time_in_seconds and current_time_in_seconds <= (configs.swap_due_timestamp_seconds + configs.swap_start_timestamp_seconds)) {
                    _swap_status := Trie.put(_swap_status, Helper.keyT(arg.canister_id), Text.equal, {
                        running = true;
                        is_successfull = null;
                    }).0;
    ...
    public shared({caller}) func participate_in_token_swap(arg : { canister_id : Text; amount : ICP.Tokens; blockIndex : Nat64; }) : async (Result.Result<Text, Text>) {
        ...
        switch (await queryIcpTx_(arg.blockIndex, swap_canister_id, participant_id, arg.amount)) { // Check-2
            case (#ok _) {
                switch(Trie.find(_swap_participants, Helper.keyT(arg.canister_id), Text.equal)) {
                    case (?participants) {
                        switch(Trie.find(participants, Helper.keyT(participant_id), Text.equal)) {
                            case (?info) {
                                _swap_participants := Trie.put2D(_swap_participants, Helper.keyT(arg.canister_id), Text.equal, Helper.keyT(participant_id), Text.equal, {
                                    account = {
                                        owner = caller;
                                        subaccount = null;
                                    };
    ...
    public shared({caller}) func settle_swap_status_and_allocate_tokens_if_swap_successfull(arg : { canister_id : Text; }) : async (Result.Result<Text, Text>) {
        ...
        let total_icp_e8s : Nat64 = await total_icp_contributed_e8s({ canister_id = arg.canister_id; });
